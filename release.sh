...
# TODO : la version linux doit-elle embarquer la runtime windows ???
...
build_linux()
{
    ...
    index "- Linux (generic): [$ARCHIVE_LINUX]($ARCHIVE_LINUX)"
...
build_pi()
{
    ...
    local ARCHIVE_LINUX=lapp-$LAPP_VERSION-raspberrypi-$OS-$OS_VERSION-aarch64.tar.gz

    index "- Raspberry Pi ($OS $OS_VERSION): [$ARCHIVE_LINUX]($ARCHIVE_LINUX)"
